...
class OutOfOrderTest(BasicTest):
    ...
    def handle_tick(self, tick_interval):
        ...
        if self.tick_times % 50 != 0:
            ...
            self.temp_out_queue.extend(filter(lambda x: x.msg_type != 'ack', self.forwarder.out_queue))
            self.forwarder.out_queue = filter(lambda x: x.msg_type == 'ack', self.forwarder.out_queue)
        else:
